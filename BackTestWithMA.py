...
class BackTestWithMA:
    ...
    def execute(self):
        ...
        self.daily_data['target'] = self.daily_data['open'] + self.daily_data['range'].shift(1) * 0.5   
        ...
        for idx, row in self.daily_data.iterrows():
            ...
            self.buy_signal = np.where(row['low'] <= row['target'] <= row['high'] \
                and row['ma5'] < row['target'] and row['ma10'] < row['target'] , True, False)
